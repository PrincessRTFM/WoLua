...
Script(function()
	...
	print("This script has no persistent storage data unless you manually create a file for it, but in order to demonstrate the `DumpStorage()` functionality, we're going to throw a few things together.")
	...
	Script.Debug.DumpStorage()
	print("None of that is saved to disk right now - it's transient persistent storage, remaining between SCRIPT reloads but not between PLUGIN reloads.")
	...
	print("Also, \"userdata\" values - which are basically magic lua values that point at plugin-internal C# things - are given special labels to make it clear that they aren't ordinary lua tables.")
end)
